...
int main() {
    ...
    refresh();
    memory_win = newwin(MEMORY_HEIGH + 2, MEMORY_WIDTH - 1, 1, 1);
    accumulator_win = newwin(ACCUMULATOR_HEIGH, ACCUMULATOR_WIDTH, 1, MEMORY_WIDTH + 5);
    decoded_win = newwin(DECODED_HEIGH, DECODED_WIDTH, MEMORY_HEIGH + 3, 1 );
    ...
    cache_win = newwin(CACHE_HEIGH, CACHE_WIDTH,BIG_HEIGH + ACCUMULATOR_HEIGH + COMMAND_HEIGH + 1, 1);
    terminal_win = newwin(TERMINAL_HEIGH, TERMINAL_WIDTH, CACHE_HEIGH + BIG_HEIGH + ACCUMULATOR_HEIGH + COMMAND_HEIGH + 1, 1);
    ...
    { 
        ...
        {
            case N_KEY_UP:
                current_address = (current_address - 10 + MEMORY_SIZE) % MEMORY_SIZE;
                break;
            case N_KEY_DOWN:
                current_address = (current_address + 10) % MEMORY_SIZE; 
                break;
            case N_KEY_LEFT:
            ...
            case N_KEY_R:{
                    ...
                    for(int i = 0; i < MEMORY_SIZE; i++ ) {
                        ...
                        printKeys(key_win);
                        printCounters (counter_win);
                        printCommand(data[current_address],command_win);
